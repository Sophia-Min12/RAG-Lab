...
The first dependency in this repo. Days 1-5 need nothing but the standard
library; from here a model gets downloaded and a GPU starts to sound
useful. The question is not "do embeddings work" — it is whether they beat
Day 5's 94% by enough to pay for themselves.

On the handbook's own questions: 100% against 94%, and at one chunk size
the model **loses**. Six points for a dependency, a download, and ninety
times the query cost is a thin case.

Then the same seventeen questions, with the same seventeen answers, worded
the way somebody who had not read the handbook would ask them. BM25 falls
to 35%. The model does not move. That gap is the entire product:
embeddings buy vocabulary mismatch and nothing else, so "are they worth
it" is a question about your users rather than your retriever.

One thing here fails silently, in the way this repo keeps finding. A chunk
longer than the model's token limit is **truncated with no error at all** —
two chunks differing only past the limit receive byte-identical
embeddings, and a fact in the tail is simply not in the index.
"""
...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print("the question this day has to answer")
    print("=" * 72)
    print("  Day 5 reached 94% with BM25: no training, no download, 0.9 ms for")
    print("  seventeen queries. An embedding model has to beat that by enough")
    print("  to pay for a dependency, a model download and its compute.")

    if not EMBEDDINGS_AVAILABLE:
        print()
        print("  sentence-transformers is not installed, so nothing below ran.")
        print("  Days 1-5 need nothing but the standard library. Install with")
        print("    pip install sentence-transformers")
        raise SystemExit(0)

    print()
    print("  first, why the handbook questions are easy for BM25:")
    for label, questions in (("original", QUESTIONS),
        ...
        print(f"    {label:<12} {sum(overlaps) / len(overlaps):>4.0%} of query words "
              f"appear in the answer's own sentence")
    print()
    print("  the paraphrased set asks the SAME seventeen questions with the")
    print("  same seventeen answers, worded the way somebody who has not read")
    print("  the handbook would ask. Nothing about them is harder.")
    print(f"    original:    {QUESTIONS[12].query!r}")
    print(f"    paraphrased: {PARAPHRASED_QUESTIONS[12].query!r}")
    print(f"    both answer: {QUESTIONS[12].answer!r}")
    ...
    index = DenseIndex(chunks, model=model)
    print(f"  {DEFAULT_MODEL}")
    print(f"  {index.dimension} dimensions, {index.token_limit} token limit, "
          f"loaded in {load_seconds:.1f}s")
    print()
    print(f"  {'':<8}{'original questions':>24}{'paraphrased':>20}")
    print(f"  {'size':<8}{'BM25':>12}{'dense':>12}{'BM25':>10}{'dense':>10}")
    for size in SIZES:
        sized = build_index(documents, size, "sentence")
        dense = DenseIndex(sized, model=model)
        row = (f"{evaluate_bm25(sized, QUESTIONS, budget=600)['hit']:>12.0%}"
               f"{evaluate_dense(dense, QUESTIONS)['hit']:>12.0%}"
               f"{evaluate_bm25(sized, PARAPHRASED_QUESTIONS, budget=600)['hit']:>10.0%}"
               f"{evaluate_dense(dense, PARAPHRASED_QUESTIONS)['hit']:>10.0%}")
        print(f"  {size:<8}{row}")

    print()
    print("  on the original questions the model is worth about six points -")
    print("  100% against 94% at the smaller sizes, and it LOSES at 240. For")
    print("  a dependency, a download and the cost measured below, that is a")
    print("  thin case, and on this evidence alone the honest recommendation")
    print("  would be to keep BM25.")
    print()
    print("  on the paraphrased questions BM25 falls to a third and the model")
    print("  does not move. Same answers, same documents, different words.")
    print("  That gap is the entire product: embeddings buy vocabulary")
    print("  mismatch, and nothing else here.")
    print()
    print("  so 'are embeddings worth it' has no general answer. It depends on")
    print("  whether the people asking use the words the documents use, which")
    print("  is a property of your users and not of your retriever.")
    ...
    start = time.perf_counter()
    for question in QUESTIONS:
        sparse.search_within_budget(question.query, budget=600)
    sparse_seconds = time.perf_counter() - start

    dense = DenseIndex(chunks, model=model)
    start = time.perf_counter()
    for question in QUESTIONS:
        dense.search_within_budget(question.query, budget=600)
    dense_seconds = time.perf_counter() - start

    print(f"  {len(QUESTIONS)} queries over {len(chunks)} chunks:")
    print(f"    BM25   {sparse_seconds * 1000:8.1f} ms")
    print(f"    dense  {dense_seconds * 1000:8.1f} ms   "
          f"({dense_seconds / sparse_seconds:.0f}x)")
    print(f"    plus {load_seconds:.1f}s to load the model, once")
    print()
    print("  and the index itself:")
    print(f"    BM25   {len(sparse.postings)} posting lists, built from counts")
    print(f"    dense  {index.matrix.shape[0]} x {index.matrix.shape[1]} floats "
          f"= {index.matrix.nbytes / 1024:.0f} KB, and every one of them")
    print("           has to be recomputed if the model changes")

    print()
    print("=" * 72)
    print("the failure with no symptom")
    print("=" * 72)
    overlong = _text_chunk(_overlong_chunk())
    probe = DenseIndex([overlong], model=model)
    tokens = probe.token_count(overlong.text)
    print(f"  a chunk of {len(overlong.text)} characters = {tokens} tokens, "
          f"against a limit of {probe.token_limit}")
    print(f"  it ends with: {overlong.text[-42:]!r}")
    print()
    for pad in (0, 100, 300, 600):
        text = _overlong_chunk(pad)
        single = DenseIndex([_text_chunk(text)], model=model)
        score = single.search("what is the emergency contact code", k=1)[0][1]
        print(f"    {pad:>4} padding words -> {single.token_count(text):>5} tokens, "
              f"similarity to the question {score:.3f}")
    print()
    print("  the last two rows are identical. Everything past the token limit")
    print("  was dropped, so the two chunks are the same chunk as far as the")
    print("  index is concerned, and the word ZEBRA is not in either of them.")
    print("  No exception, no warning, no empty result.")
    print()
    print(f"  chunks over the limit in the handbook index: "
          f"{len(index.truncated_chunks())}")
    print("  none here, because Day 3 settled on chunks far smaller than 256")
    print("  tokens. But the limit is a hard ceiling on chunk size that Days 3")
    print("  and 4 never mentioned, because until today nothing had one.")

    print()
    print("=" * 72)
    print("where this leaves the baseline")
    print("=" * 72)
    small = build_index(documents, 80, "sentence")
    small_dense = DenseIndex(small, model=model)
    print(f"  {'':<8}{'original':>10}{'paraphrased':>14}{'17 queries':>13}")
    print(f"  {'BM25':<8}"
          f"{evaluate_bm25(small, QUESTIONS, budget=600)['hit']:>10.0%}"
          f"{evaluate_bm25(small, PARAPHRASED_QUESTIONS, budget=600)['hit']:>14.0%}"
          f"{sparse_seconds * 1000:>11.1f} ms")
    print(f"  {'dense':<8}"
          f"{evaluate_dense(small_dense, QUESTIONS)['hit']:>10.0%}"
          f"{evaluate_dense(small_dense, PARAPHRASED_QUESTIONS)['hit']:>14.0%}"
          f"{dense_seconds * 1000:>11.1f} ms")
    print()
    print("  neither dominates on cost-adjusted terms, and both fail")
    print("  differently. Day 9 stops choosing and combines them.")
