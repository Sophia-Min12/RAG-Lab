...
Level 1 built the index. This is the first thing that searches it, and it
is deliberately not a neural model: BM25 is thirty years old, needs no
training, no GPU and no dependency beyond the standard library, and it is
still the number a retrieval system has to beat before anything more
expensive is worth its cost.

Its three ideas each repair a specific failure of the scorers Day 3 used:

- **IDF** — a term in every chunk distinguishes nothing. Day 3's scorers
  weighted "the" and "forfeited" identically; here they differ elevenfold.
- **Term saturation** — the ninth mention of a word is not worth the
  first. ``score_occurrences`` believed the hundredth was worth a hundred.
- **Length normalization** — the bias Day 3 measured directly, with ``b``
  as the knob. ``b=0`` is Day 3's biased scorer exactly.

Two results here are worth more than the implementation. At the universal
default ``b=0.75``, a chunk that merely repeats the query five times still
outranks the short chunk that answers it — the default is a compromise,
not a cure. And on this handbook ``b`` changes nothing at all, because
sentence chunks vary in length by a coefficient of 0.16 and length
normalization has nothing to normalize. Mixing in Day 4's tables and code
raises that to 0.80, and the effect appears.

BM25 reaches 94% here. That is the bar Day 6's embedding model has to
clear to justify its dependency, its download and its compute.
"""
...
#: Two chunks built to isolate the length bias: one short chunk that
#: answers the question, one long chunk that repeats the query terms and
#: answers nothing.
PADDING_DEMO = (
    "Core hours are 10:00 to 16:00.",
    "Core hours matter. Employees ask about core hours often. The core "
    "hours policy is discussed in the core hours section of the core "
    "hours appendix, which covers core hours in detail.",
)
...
if __name__ == "__main__":
    ...
    short, padded = (_text_chunk(text) for text in PADDING_DEMO)

    print("=" * 70)
    print("1. the three ideas, one at a time")
    print("=" * 70)
    print("  inverse document frequency - a term in every chunk says nothing:")
    sample = [_text_chunk(t) for t in ("the policy on leave", "the policy on expenses",
                                       "the policy on equipment",
                                       "the forfeited balance rule")]
    index = BM25(sample)
    for term in ("the", "policy", "forfeited"):
        print(f"    {term!r:>12}  df {index.document_frequency(term)}/4  "
              f"idf {index.idf(term):.3f}")
    print("    Day 3's scorers weighted all three the same.")
    print()
    print("  term saturation - the ninth mention is not worth the first:")
    print(f"    {'tf':>5}{'contribution':>14}{'gain over tf-1':>16}")
    previous = 0.0
    for frequency in (1, 2, 3, 4, 5):
        value = saturation(frequency)
        print(f"    {frequency:>5}{value:>14.3f}{value - previous:>16.3f}")
        previous = value
    print(f"    {'...':>5}")
    for frequency in (10, 100, 10_000):
        print(f"    {frequency:>5}{saturation(frequency):>14.3f}")
    print(f"    the ceiling is k1 + 1 = {1.5 + 1:.1f}, approached and never passed.")
    print("    score_occurrences believed the hundredth mention was worth")
    print("    a hundred firsts; here it is worth less than the second.")

    print()
    print("=" * 70)
    print("2. length normalization, which is the bias Day 3 measured")
    print("=" * 70)
    print(f"  short  ({len(short.text):>3} chars): {short.text!r}")
    print(f"  padded ({len(padded.text):>3} chars): {padded.text[:52]!r}...")
    print(f"  query: {query!r}   (the answer is only in the short one)")
    print()
    print(f"  {'b':>6}{'short':>10}{'padded':>10}   ranked first")
    for setting in (0.0, 0.25, 0.5, 0.75, 1.0):
        index = BM25([short, padded], b=setting)
        first, second = index.score(query, 0), index.score(query, 1)
        print(f"  {setting:>6.2f}{first:>10.3f}{second:>10.3f}   "
              f"{'short' if first > second else 'padded'}")
    print()
    print("  b=0 is Day 3's score_occurrences: no correction, padding wins.")
    print("  b=1 divides by length and the short chunk finally wins.")
    print()
    print("  and the part worth saying out loud: the universal default is")
    print("  b=0.75, and at b=0.75 the padded chunk STILL RANKS FIRST. The")
    print("  default is a compromise tuned for ordinary collections, not a")
    print("  cure for a chunk that repeats the query five times. 'BM25 handles")
    print("  length' is true on average and not true here.")

    print()
    print("=" * 70)
    print("3. so does b matter on the actual handbook?")
    print("=" * 70)
    print("  chunk length spread, which is what b has to work with:")
    print(f"    {'size':>5}{'strategy':>11}{'mean':>7}{'sd':>6}{'cv':>7}")
    for size in (80, 160, 320):
        for strategy in ("sentence", "fixed"):
            lengths = [len(c.text) for c in build_index(documents, size, strategy)]
            mean = statistics.mean(lengths)
            deviation = statistics.pstdev(lengths)
            print(f"    {size:>5}{strategy:>11}{mean:>7.0f}{deviation:>6.0f}"
                  f"{deviation / mean:>7.2f}")
    print()
    print("  hit rate at a 600-character budget, by b:")
    print(f"    {'size':>5}" + "".join(f"{f'b={b}':>9}" for b in (0.0, 0.5, 1.0)))
    for size in SIZES:
        chunks = build_index(documents, size, "sentence")
        row = "".join(f"{evaluate_bm25(chunks, QUESTIONS, b=b, budget=600)['hit']:>9.0%}"
                      for b in (0.0, 0.5, 1.0))
        print(f"    {size:>5}{row}")
    print()
    print("  nothing moves. That is a real result, not a broken measurement:")
    print("  b acts only through |d|/avgdl, and sentence chunks of one")
    print("  handbook have a coefficient of variation of 0.16 - every chunk")
    print("  is about average length, so the correction has nothing to")
    print("  correct. A knob everyone tunes, doing nothing, for a reason the")
    print("  column above states.")
    print()
    ...
    print(f"  now mix in Day 4's tables and code - {len(mixed)} chunks, "
          f"{min(lengths)} to {max(lengths)} characters, cv {spread:.2f}:")
    print(f"    {'b':>6}{'hit@3':>8}")
    for setting in (0.0, 0.25, 0.5, 1.0):
        index = BM25(mixed, b=setting)
        hits = sum(any(question.answer in chunk.text
                       for chunk, _ in index.search(question.query, k=3))
                   for question in QUESTIONS)
        print(f"    {setting:>6.2f}{hits / len(QUESTIONS):>8.0%}")
    print("  one question's worth, which is the honest size of the effect on a")
    print("  corpus this small - but it appears exactly where the theory says.")

    print()
    print("=" * 70)
    print("4. the baseline")
    print("=" * 70)
    print(f"  {'size':>5}{'occurrences':>14}{'coverage':>11}{'BM25':>8}")
    for size in SIZES:
        chunks = build_index(documents, size, "sentence")
        ...
        c = evaluate_bm25(chunks, QUESTIONS, budget=600)
        print(f"  {size:>5}{a['hit']:>14.0%}{b['hit']:>11.0%}{c['hit']:>8.0%}")
    print()
    print("  BM25 is never worse and is better where Day 3's two scorers")
    print("  disagreed - at 320 characters it scores 94% against 82% and 88%.")
    print("  It also removes the disagreement itself, which was the actual")
    print("  problem: one principled scorer instead of two defensible ones")
    print("  that contradict each other.")
    print()
    best = max(evaluate_bm25(build_index(documents, size, "sentence"),
                             QUESTIONS, budget=600)["hit"] for size in SIZES)
    print(f"  best BM25 hit rate at a 600-character budget: {best:.0%}")
    print("  that number is the bar. Day 6 loads a neural embedding model,")
    print("  which costs a dependency, a download and two orders of magnitude")
    print("  more compute per query, and it has to beat 94% to justify any of")
    print("  it. A great many RAG systems never run this comparison.")

    print()
    print("=" * 70)
    print("5. the inverted index")
    print("=" * 70)
    chunks = build_index(documents, 80, "sentence")
    index = BM25(chunks)
    print(f"  {len(chunks)} chunks, {len(index.postings)} distinct terms")
    for question in QUESTIONS[:4]:
        touched = set()
        for term in tokenize(question.query):
            touched.update(position for position, _ in index.postings.get(term, ()))
        print(f"    {len(touched):>3}/{len(chunks)} chunks scored for "
              f"{question.query[:44]!r}")
    print("  a query only touches chunks that share a term with it. At this")
    print("  size that saves nothing; it is the reason BM25 still runs on")
    print("  collections where computing an embedding per document per query")
    print("  would be impossible.")
