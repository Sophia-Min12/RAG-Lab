...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print("1. before fusing anything: is there anything to fuse?")
    print("=" * 72)
    print(f"  {'question set':<14}{'BM25':>7}{'dense':>7}{'union':>7}"
          f"{'BM25 only':>11}{'dense only':>12}")
    for label, questions in (("original", QUESTIONS),
                             ("paraphrased", PARAPHRASED_QUESTIONS)):
        counts = complementarity(retriever, questions)
        print(f"  {label:<14}{counts['sparse']:>7}{counts['dense']:>7}"
              f"{counts['union']:>7}{counts['sparse_only']:>11}"
              f"{counts['dense_only']:>12}")
    print()
    print("  on the original questions each retriever finds exactly one the")
    print("  other misses, so the union is 17/17 and fusion has somewhere to")
    print("  go. On the paraphrased set BM25 finds NOTHING dense missed - the")
    print("  union equals dense alone - so no combination rule can beat dense")
    print("  there, and some will do worse.")
    print()
    print("  that table costs two retrievals per question and settles whether")
    print("  the rest of this day is worth doing. It is the step most hybrid")
    print("  write-ups skip.")

    print()
    print("=" * 72)
    print("2. score fusion: put both on [0,1] and take a weighted sum")
    print("=" * 72)
    print(f"  {'alpha':>7}{'original':>11}{'paraphrased':>14}{'mean':>8}")
    for alpha in (0.0, 0.25, 0.5, 0.75, 1.0):
        first = evaluate_hybrid(retriever, QUESTIONS, method="score", alpha=alpha)
        second = evaluate_hybrid(retriever, PARAPHRASED_QUESTIONS,
                                 method="score", alpha=alpha)
        print(f"  {alpha:>7.2f}{first:>11.0%}{second:>14.0%}{(first + second) / 2:>8.0%}")
    print()
    print("  at alpha 0.25 to 0.75 the original set reaches 100%, which")
    print("  neither retriever manages alone. That is fusion doing exactly")
    print("  what it promises, on the set where the complementarity table")
    print("  said it could.")
    print()
    print("  the normalization underneath is shakier than it looks. min-max")
    print("  maps scores using the minimum and maximum OF THE RESULT LIST, so")
    print("  the same chunk with the same BM25 score normalizes differently")
    print("  depending on what else came back:")
    shallow = [score for _, score in retriever.search(QUESTIONS[0].query, k=3,
                                                      method="sparse")]
    deep = [score for _, score in retriever.search(QUESTIONS[0].query, k=8,
                                                   method="sparse")]
    print(f"    raw BM25 scores, top 3       {[round(s, 3) for s in shallow]}")
    print(f"    normalized within top 3      {[round(s, 3) for s in min_max_normalize(shallow)]}")
    print(f"    the same three within top 8  "
          f"{[round(s, 3) for s in min_max_normalize(deep)[:3]]}")
    print("    a score that changes when you ask for more results is not a")
    print("    property of the chunk.")

    print()
    print("=" * 72)
    print("3. rank fusion: ignore the scores entirely")
    print("=" * 72)
    print("  RRF sums 1/(constant + rank), so there is nothing to normalize.")
    print(f"  {'constant':>9}{'original':>11}{'paraphrased':>14}{'mean':>8}")
    for constant in (0, 1, 5, 20, 60, 200):
        first = evaluate_hybrid(retriever, QUESTIONS, method="rrf", constant=constant)
        second = evaluate_hybrid(retriever, PARAPHRASED_QUESTIONS, method="rrf",
                                 constant=constant)
        marker = "   <- the standard default" if constant == 60 else ""
        print(f"  {constant:>9}{first:>11.0%}{second:>14.0%}"
              f"{(first + second) / 2:>8.0%}{marker}")
    print()
    print("  the published default of 60 is the worst setting on this data,")
    print("  by 41 points on the paraphrased set. The reason is what the")
    print("  constant does: at 60, ranks 1 through 20 differ in weight by")
    print("  under 25%, so RRF is close to counting 'appeared in either")
    print("  list'. That is the right behaviour when the inputs are several")
    print("  systems of comparable quality - which is what the RRF paper was")
    print("  fusing - and the wrong behaviour when one list is nearly noise.")
    print()
    print("  at constant 0, rank 1 is worth twice rank 2 and a confident top")
    print("  hit can outvote a bad list. Still not enough to beat dense alone")
    print("  on the paraphrased set, because RRF has no way to know that one")
    print("  of its two inputs is worthless.")

    print()
    print("=" * 72)
    print("4. the comparison that matters")
    print("=" * 72)
    print(f"  {'method':<24}{'original':>11}{'paraphrased':>14}{'mean':>8}")
    for label, kwargs in (("BM25 alone", {"method": "sparse"}),
                          ("dense alone", {"method": "dense"}),
                          ("score fusion, alpha=0.5", {"method": "score", "alpha": 0.5}),
                          ("RRF, constant=60", {"method": "rrf"}),
                          ("RRF, constant=0", {"method": "rrf", "constant": 0})):
        first = evaluate_hybrid(retriever, QUESTIONS, **kwargs)
        second = evaluate_hybrid(retriever, PARAPHRASED_QUESTIONS, **kwargs)
        print(f"  {label:<24}{first:>11.0%}{second:>14.0%}"
              f"{(first + second) / 2:>8.0%}")
    print()
    print("  the conclusion this corpus supports: tuned score fusion beats")
    print("  everything on the original set and matches dense alone on the")
    print("  mean, while RRF at its standard setting is worse than either")
    print("  retriever by itself.")
    print()
    print("  'always use hybrid' is not what these numbers say. What they say")
    print("  is: measure the complementarity first. Where both retrievers")
    print("  contribute, fusion pays. Where one is carrying the other, fusion")
    print("  is a way to make it worse - and rank fusion, which cannot tell a")
    print("  good list from a bad one, is the most likely to.")
