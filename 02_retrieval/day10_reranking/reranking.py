...
A bi-encoder embeds the query and the chunk separately, so the chunk's
vector knows nothing about the query — which is exactly why it can be
precomputed. A cross-encoder gives that up: one forward pass per
``(query, chunk)`` pair, attention running across both, nothing storable.

It works. Dense retrieval plus reranking answers **every question in this
corpus**, on both the original and the paraphrased sets — the first
configuration in this repo that does.

And it is bounded in a way worth stating before the gain: every reranked
number here equals its first-stage ceiling **exactly**. The cross-encoder
extracts everything the shortlist made available and not one answer more.
Where the first stage is BM25 on paraphrased questions, that ceiling is
41% at any depth including the whole index — a chunk sharing no term with
the query is never a candidate, so there is nothing to reorder.

Reranking fixes ordering. It never fixes recall.
"""
...
if __name__ == "__main__":
    ...
    reranker = Reranker(model=cross_encoder)

    print("=" * 72)
    print("1. what a cross-encoder is")
    print("=" * 72)
    print("  Day 6's bi-encoder embeds the query and the chunk separately.")
    print("  The chunk's vector knows nothing about the query - it was")
    print("  computed first, which is exactly why it can be stored.")
    print()
    print("  a cross-encoder runs both through one model together, so")
    print("  attention crosses between them:")
    right = "Employees working remotely must be reachable during core hours, which are 10:00 to 16:00."
    wrong = "Laptops are replaced on a three year cycle, or earlier if a fault cannot be repaired."
    for query in ("what are the core hours", "when am I expected to be online"):
        scores = reranker.score(query, [right, wrong])
        print(f"    {query!r}")
        print(f"      {scores[0]:>8.2f}  the core-hours sentence")
        print(f"      {scores[1]:>8.2f}  the laptop sentence")
    print()
    print("  the scores are logits, comparable within one query and")
    print("  meaningless across queries - note the paraphrase scores the")
    print("  right sentence NEGATIVELY in absolute terms while still")
    print("  ranking it far above the wrong one. Only the order is usable,")
    print("  which is why this reranks and does not threshold.")

    print()
    print("=" * 72)
    print("2. the ceiling, before any gain is measured")
    print("=" * 72)
    print("  a reranker reorders a shortlist. It cannot add to it.")
    print()
    print(f"  {'depth':>6}{'BM25 orig':>12}{'BM25 para':>12}"
          f"{'dense orig':>13}{'dense para':>13}")
    for depth in (3, 5, 10, 20, 26):
        cells = ""
        for method in ("sparse", "dense"):
            for questions in (QUESTIONS, PARAPHRASED_QUESTIONS):
                width = 12 if method == "sparse" else 13
                value = first_stage_recall(retriever, questions, depth, method=method)
                cells += f"{value:>{width}.0%}"
        print(f"  {depth:>6}{cells}")
    print()
    print("  BM25 on the paraphrased set tops out at 41% and stays there at")
    print("  depth 26, which is the entire index. That is not a ranking")
    print("  problem that a better reranker could fix:")
    sparse_index = BM25(chunks)
    counts = [len(sparse_index.search(q.query, k=len(chunks)))
              for q in PARAPHRASED_QUESTIONS]
    empty = sum(1 for count in counts if count == 0)
    print(f"    BM25 returns {sum(counts) / len(counts):.1f} of {len(chunks)} chunks "
          f"on average for these queries,")
    print(f"    and {empty} of them "
          f"{'returns' if empty == 1 else 'return'} nothing at all.")
    print("    a chunk sharing no term with the query is not a candidate at")
    print("    ANY depth, so there is nothing for a reranker to reorder.")

    print()
    print("=" * 72)
    print("3. what reranking buys")
    print("=" * 72)
    print(f"  {'first stage':<14}{'set':<14}{'plain':>8}{'reranked':>11}{'ceiling':>10}")
    for method in ("sparse", "dense"):
        for label, questions in (("original", QUESTIONS),
                                 ("paraphrased", PARAPHRASED_QUESTIONS)):
            plain = evaluate_hybrid(retriever, questions, k=3, method=method)
            reranked = evaluate_reranked(retriever, reranker, questions,
                                         depth=20, k=3, method=method)
            ceiling = first_stage_recall(retriever, questions, 20, method=method)
            print(f"  {method:<14}{label:<14}{plain:>8.0%}{reranked:>11.0%}"
                  f"{ceiling:>10.0%}")
    print()
    print("  every reranked number equals its ceiling exactly. The")
    print("  cross-encoder is extracting everything the first stage made")
    print("  available, in all four cases, so the only thing limiting it is")
    print("  first-stage recall.")
    print()
    print("  dense + reranking reaches 100% on both question sets - the")
    print("  first configuration in this repo that does.")

    print()
    print("  the same thing against shortlist depth:")
    print(f"    {'depth':>6}{'ceiling':>10}{'reranked':>11}")
    for depth in (3, 5, 10, 20, 26):
        ceiling = first_stage_recall(retriever, PARAPHRASED_QUESTIONS, depth,
                                     method="dense")
        got = evaluate_reranked(retriever, reranker, PARAPHRASED_QUESTIONS,
                                depth=depth, k=3, method="dense")
        print(f"    {depth:>6}{ceiling:>10.0%}{got:>11.0%}")
    print("    the two columns are identical at every depth. Deepening the")
    print("    shortlist helps only while it raises the ceiling.")

    print()
    print("=" * 72)
    print("4. what it costs")
    print("=" * 72)
    print("  one forward pass per (query, chunk) pair, and nothing can be")
    print("  precomputed, because the pair does not exist until the query does.")
    print()
    print(f"  {'pairs':>7}{'ms':>10}{'ms/pair':>10}")
    costs = {}
    for count in (1, 5, 10, 20, 50, 100):
        pairs = [("what are the core hours", chunks[i % len(chunks)].text)
                 for i in range(count)]
        cross_encoder.predict(pairs[:1], show_progress_bar=False)
        start = time.perf_counter()
        cross_encoder.predict(pairs, show_progress_bar=False)
        elapsed = (time.perf_counter() - start) * 1000
        costs[count] = elapsed
        print(f"  {count:>7}{elapsed:>10.1f}{elapsed / count:>10.2f}")
    print()
    sparse_index = BM25(chunks)
    start = time.perf_counter()
    for question in QUESTIONS:
        sparse_index.search(question.query, k=20)
    bm25_ms = (time.perf_counter() - start) / len(QUESTIONS) * 1000
    print("  linear in the shortlist, so the depth is a latency budget:")
    print(f"    BM25, top 20                {bm25_ms:>8.2f} ms per query")
    print(f"    reranking 20 candidates     {costs[20]:>8.1f} ms per query")
    print(f"    reranking 50 candidates     {costs[50]:>8.1f} ms per query")
    print(f"    ratio                       {costs[20] / bm25_ms:>8.0f}x")
    print()
    print("  reranking the whole index is affordable here only because the")
    print("  index is 26 chunks. At a million it would be impossible, which")
    print("  is the entire reason this is a second stage and not a first one.")

    print()
    print("=" * 72)
    print("what Level 2 established")
    print("=" * 72)
    print("  Day 5   BM25, 94% - and a default b that does not fix padding")
    print("  Day 6   embeddings buy vocabulary mismatch and nothing else")
    print("  Day 7   exact search is one BLAS call; the wall is memory")
    print("  Day 8   approximate search trades recall for a constant factor")
    print("          that a bad memory layout eats entirely")
    print("  Day 9   fuse only where the retrievers are complementary; RRF's")
    print("          standard constant made things worse here")
    print("  Day 10  reranking fixes ordering and never recall")
    print()
    print("  the configuration that answers every question in this corpus is")
    print("  dense retrieval over a shortlist, reranked - and it costs about")
    print("  70 ms per query against BM25's 0.04, for six points on questions")
    print("  phrased the way the documents are.")
    print()
    print("  Level 3 stops trusting hit@3 and asks what these numbers mean.")
