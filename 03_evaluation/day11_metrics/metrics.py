...
Every number in Level 2 was hit@3 on seventeen questions. This day asks
what that measured, and finds three things.

**MAP and MRR are the same number here.** This corpus has exactly one
relevant chunk per question, and average precision over a single relevant
item *is* the reciprocal rank. Printing both looks like two pieces of
evidence and is one.

**hit@3 was hiding a large difference.** On paraphrased questions score
fusion scores 0.824 to dense's 0.882 — close, Day 9 said. By MRR it is
0.529 to 0.814, which is not close at all: score fusion finds the answer
and ranks it badly, and a binary hit cannot tell first place from third.

**Two metrics can agree on every query and disagree about the system.**
With one relevant chunk, MRR and nDCG are both functions of its rank
alone, so they cannot differ on a single question. Averaged over
seventeen, MRR prefers RRF and nDCG prefers score fusion — over the same
result lists — because ``1/rank`` and ``1/log2(rank+1)`` weight a
rank-15 disaster and a rank-2 near-miss differently.

And the last section undoes some of the rest: at seventeen questions the
95% interval on 94% runs from 83% to 105%, so most of the distinctions
Level 2 drew are inside the noise. Day 12 fixes the sample size.
"""
...
if __name__ == "__main__":
    ...
    reranker = Reranker(model=CrossEncoder(DEFAULT_CROSS_ENCODER))

    print("=" * 72)
    print("1. what each metric is asking")
    print("=" * 72)
    example = [0, 1, 0, 1, 0]
    print(f"  one query, judgements by rank: {example}")
    print(f"    hit@3          {hit_at_k(example, 3):.3f}   is anything relevant in the top 3")
    print(f"    precision@3    {precision_at_k(example, 3):.3f}   what fraction of the top 3 is relevant")
    print(f"    recall@3       {recall_at_k_judged(example, 3, 2):.3f}   what fraction of all relevant it found")
    print(f"    reciprocal rank{reciprocal_rank(example):>7.3f}   how far down the first one was")
    print(f"    average prec.  {average_precision(example):.3f}   how early ALL of them were")
    print(f"    nDCG@3         {ndcg_at_k(example, 3, 2):.3f}   the same, discounted by log of rank")
    print()
    print("  they are not interchangeable and they are not a panel of")
    print("  independent judges either. Each is one number computed from the")
    print("  same list of judgements.")

    print()
    print("=" * 72)
    print("2. Level 2, rescored")
    print("=" * 72)
    configurations = [
        ("BM25", {"method": "sparse"}, None, 3),
        ("dense", {"method": "dense"}, None, 3),
        ("score fusion", {"method": "score", "alpha": 0.5}, None, 3),
        ("RRF c=60", {"method": "rrf"}, None, 3),
        ("dense + rerank", {"method": "dense"}, reranker, 20),
    ]
    for label, questions in (("original", QUESTIONS),
                             ("paraphrased", PARAPHRASED_QUESTIONS)):
        print(f"  {label} questions, k=3:")
        print(f"    {'method':<16}{'hit@3':>8}{'P@3':>8}{'MRR':>8}{'MAP':>8}{'nDCG@3':>9}")
        for name, kwargs, rank_model, depth in configurations:
            scores = evaluate_all(retriever, questions, k=3, depth=depth,
                                  reranker=rank_model, **kwargs)
            print(f"    {name:<16}{scores['hit']:>8.3f}{scores['precision']:>8.3f}"
                  f"{scores['mrr']:>8.3f}{scores['map']:>8.3f}{scores['ndcg']:>9.3f}")
        print()
    print("  MAP and MRR are identical in every row, because this corpus has")
    print("  exactly one relevant chunk per question and average precision")
    print("  over a single relevant item IS the reciprocal rank. Printing")
    print("  both suggests two pieces of evidence where there is one.")
    print()
    print("  precision@3 tops out at 0.333 for the same reason: one relevant")
    print("  chunk in three retrieved. It is measuring the value of k, not")
    print("  the retriever, and on this corpus it is nearly useless.")

    print()
    print("=" * 72)
    print("3. where hit@3 was hiding something")
    print("=" * 72)
    print("  paraphrased questions, score fusion against dense:")
    for name, kwargs in (("dense", {"method": "dense"}),
                         ("score fusion", {"method": "score", "alpha": 0.5})):
        scores = evaluate_all(retriever, PARAPHRASED_QUESTIONS, k=3, depth=3, **kwargs)
        print(f"    {name:<14} hit@3 {scores['hit']:.3f}   MRR {scores['mrr']:.3f}")
    print()
    print("  Day 9 read those hit rates as 88% against 82% and called it")
    print("  close. By MRR it is 0.814 against 0.529, which is not close at")
    print("  all: score fusion finds the answer and ranks it badly. hit@3")
    print("  cannot see the difference between first place and third.")

    print()
    print("=" * 72)
    print("4. two metrics that agree on every query and disagree on the system")
    print("=" * 72)
    print("  with one relevant chunk, both are functions of its rank alone:")
    print(f"    {'rank':>5}{'1/rank':>10}{'1/log2(rank+1)':>17}")
    for rank in (1, 2, 3, 5, 15):
        print(f"    {rank:>5}{1 / rank:>10.4f}{1 / math.log2(rank + 1):>17.4f}")
    print()
    print("  so they cannot disagree about a query. They can still disagree")
    print("  about an average. Measured over the whole index rather than a")
    print("  top-3, so a badly ranked answer is visible instead of truncated:")
    print()
    for name, kwargs in (("score fusion", {"method": "score", "alpha": 0.5}),
                         ("RRF c=60", {"method": "rrf"})):
        ranks = answer_ranks(retriever, QUESTIONS, **kwargs)
        found = [rank for rank in ranks if rank]
        distribution = dict(sorted(Counter(found).items()))
        mrr = sum(1 / rank for rank in found) / len(ranks)
        ndcg = sum(1 / math.log2(rank + 1) for rank in found) / len(ranks)
        print(f"    {name:<14} answer ranks {distribution}")
        print(f"    {'':<14} MRR {mrr:.4f}    nDCG {ndcg:.4f}")
    print()
    print("  MRR prefers RRF. nDCG prefers score fusion. Same result lists,")
    print("  same judgements, opposite verdicts - because 1/rank punishes")
    print("  that single rank-15 result down to 0.067 while 1/log2(rank+1)")
    print("  leaves it at 0.25, and nDCG pays more for a rank 2 than MRR")
    print("  does. Averaging a different curve over the same ranks is enough")
    print("  to swap the winner.")

    print()
    print("=" * 72)
    print("5. and none of it survives the sample size")
    print("=" * 72)
    print(f"  every number in this repo comes from {len(QUESTIONS)} questions.")
    print(f"  {'hits':>6}{'rate':>9}{'95% interval':>22}")
    for hits in (10, 14, 16, 17):
        low, high = confidence_interval(hits, len(QUESTIONS))
        print(f"  {hits:>4}/{len(QUESTIONS)}{hits / len(QUESTIONS):>9.0%}"
              f"{f'{low:.0%} to {high:.0%}':>22}")
    print()
    print("  16/17 and 17/17 - 94% and 100%, the difference Level 2 kept")
    print("  reporting - are not distinguishable: the interval for 16/17")
    print("  runs from 83% to 105%, and contains 100% comfortably.")
    print()
    print("  both boundary rows are nonsense in their own way. 105% is not a")
    print("  rate, and 17/17 reports an interval of zero width because the")
    print("  normal approximation computes a variance of p(1-p) = 0. Neither")
    print("  is a bug here; both are the approximation failing where the")
    print("  data is, which is precisely where Level 2 spent its time.")
    print()
    print("  the MRR gap in section 4 is 0.004. On seventeen questions that")
    print("  is one answer moving from rank 2 to rank 1. Reporting it to four")
    print("  decimals implies a precision the evaluation set cannot support.")
    print()
    print("  so the honest summary of Level 2 is: dense + reranking is")
    print("  clearly better than BM25 alone, BM25 is clearly bad on")
    print("  paraphrases, and every finer distinction than that is inside")
    print("  the noise. Day 12 builds an evaluation set large enough to say")
    print("  more, without hand-labelling a thousand queries.")
