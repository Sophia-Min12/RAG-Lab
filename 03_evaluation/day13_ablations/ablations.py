...
The table is easy: take the best pipeline, remove one component, measure
the drop. Embeddings cost 0.265 MRR, chunking strategy 0.197, the
reranker 0.064, chunk size nothing at all.

Then two measurements say the table cannot be read the way it invites.

**It is not additive.** Removing the reranker and the embeddings together
costs 0.118 *more* than the sum of removing each — they were covering for
each other. Removing the embeddings and using fixed chunks costs 0.081
*less* than the sum — their damage overlaps, and a question can only be
lost once.

**The reranker is worth 0.064 or 0.279** depending only on which first
stage runs before it, because every first stage converges to the same
0.912 once it is reranked. The reranker does not improve the first
stage's ranking, it replaces it — so the better the first stage, the less
visible the reranker. The ablation table reported the smallest of those
numbers, because it measured the reranker on top of the best first stage
available.

An ablation measures the damage from removing a component *from this
pipeline, on this corpus, with these questions*. It is not a property of
the component, and a component that looks free may be one that something
else is covering for.
"""
...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print("1. the table everybody publishes")
    print("=" * 72)
    print(f"  baseline: {Pipeline().label()}, {len(everything)} questions")
    print()
    for label, questions in (("all 34 questions", everything),
                             ("paraphrased only", list(PARAPHRASED_QUESTIONS))):
        print(f"  {label}:")
        print(f"    {'ablation':<26}{'MRR':>8}{'delta':>9}{'hit@3':>8}")
        for row in ablate(documents, questions, model=model,
                          cross_encoder=cross_encoder):
            print(f"    {row['name']:<26}{row['mrr']:>8.3f}"
                  f"{row['delta']:>+9.3f}{row['hit']:>8.3f}")
        print()
    print("  read straight down: embeddings are worth the most, chunking")
    print("  strategy is worth a lot, the reranker is worth a little, and")
    print("  chunk size is worth nothing.")
    print()
    print("  two rows deserve suspicion before they deserve belief. 'large")
    print("  chunks' comes out ABOVE the baseline by 0.010, and 'tiny chunks'")
    print("  and 'hybrid instead of dense' come out exactly level with it.")
    print("  Day 11's interval on 34 questions is wider than any of those,")
    print("  so the honest reading of all three is 'no measurable effect'.")

    print()
    print("=" * 72)
    print("2. the table is not additive")
    print("=" * 72)
    print("  if each delta were a property of its component, removing two")
    print("  would cost the sum of removing each. It does not:")
    print()
    print(f"  {'pair removed':<32}{'both':>8}{'additive':>10}{'interaction':>13}")
    pairs = [("no reranker", {"rerank": False}, "no embeddings", {"method": "sparse"}),
             ("no reranker", {"rerank": False}, "fixed chunks", {"strategy": "fixed"}),
             ("no embeddings", {"method": "sparse"}, "fixed chunks",
              {"strategy": "fixed"})]
    for first_name, first, second_name, second in pairs:
        result = interaction(documents, everything, first, second, model=model,
                             cross_encoder=cross_encoder)
        print(f"  {first_name + ' + ' + second_name:<32}"
              f"{result['without_both']:>8.3f}{result['additive_prediction']:>10.3f}"
              f"{result['interaction']:>+13.3f}")
    print()
    print("  the signs go both ways, and both directions mean something.")
    print()
    print("  negative: removing the reranker and the embeddings together")
    print("  costs 0.118 MORE than the sum. Each was covering for the other,")
    print("  so measuring either alone understates it.")
    print()
    print("  positive: removing the embeddings and using fixed chunks costs")
    print("  0.081 LESS than the sum. Their damage overlaps - both lose the")
    print("  same questions, and a question can only be lost once.")

    print()
    print("=" * 72)
    print("3. what the reranker is worth depends on what precedes it")
    print("=" * 72)
    print(f"  {'first stage':<16}{'no rerank':>11}{'rerank':>9}{'gain':>9}")
    gains = {}
    for method in ("sparse", "rrf", "score", "dense"):
        without = run_pipeline(documents, everything,
                               Pipeline(method=method, rerank=False),
                               model, cross_encoder)["mrr"]
        with_it = run_pipeline(documents, everything,
                               Pipeline(method=method, rerank=True),
                               model, cross_encoder)["mrr"]
        gains[method] = with_it - without
        print(f"  {method:<16}{without:>11.3f}{with_it:>9.3f}{with_it - without:>+9.3f}")
    print()
    print(f"  the reranker is worth {min(gains.values()):+.3f} on one first stage and")
    print(f"  {max(gains.values()):+.3f} on another - a factor of "
          f"{max(gains.values()) / min(gains.values()):.1f} - and the ablation table")
    print("  above reported one of those numbers as though it were the answer.")
    print()
    print("  look at the 'rerank' column: three of the four first stages land")
    print("  on exactly 0.912. The reranker does not improve their ranking,")
    print("  it REPLACES it, so whatever order they produced stops mattering")
    print("  and only their candidate sets do. The better the first stage,")
    print("  the less visible the reranker - which is why the ablation table")
    print("  called it the cheapest component: it was measured on top of the")
    print("  best first stage available.")
    print()
    print("  BM25 is the exception, at 0.647, and Day 10 said why: its")
    print("  candidate set does not contain the answer for most paraphrased")
    print("  questions, so there is nothing to reorder. A reranker converges")
    print("  first stages that RETRIEVE the same things, and BM25 does not.")

    print()
    print("=" * 72)
    print("what an ablation actually measures")
    print("=" * 72)
    print("  not the value of a component. The damage from removing it from")
    print("  THIS pipeline, on THIS corpus, with THESE questions - and the")
    print("  three tables above show each of those qualifiers doing real")
    print("  work:")
    print()
    print("    this pipeline   the reranker is worth 0.064 or 0.279")
    print("                    depending only on what runs before it")
    print("    this corpus     chunk size is worth nothing here and Day 3")
    print("                    showed it is worth a great deal on fixed chunks")
    print("    these questions embeddings cost 0.265 over all 34 questions")
    print("                    and 0.471 over the paraphrased half alone")
    print()
    print("  a component that looks free may simply be one another component")
    print("  is covering for. The only way to see that is to remove both, and")
    print("  a single-ablation table never does.")
    print()
    print("  Level 3 is done. The retrieval side of this repo is as measured")
    print("  as a corpus of 34 questions permits, and Day 11 was clear about")
    print("  what that permits. Level 4 starts generating answers, where the")
    print("  measurement problem gets considerably worse.")
