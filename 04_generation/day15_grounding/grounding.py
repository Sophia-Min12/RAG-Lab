...
Day 2 built offsets that survive normalization. This is what they were
for: an answer that names a document, a line and a character span, and a
check that the span really contains what the system says it does.

Every grounded answer here verifies. The interesting cell is the other
one — **18% of answers are grounded and wrong**: they quote a real chunk,
the quote is accurate, and the answer does not follow from it. A citation
makes those *more* convincing rather than less, which is the honest limit
of grounding. It is a property of provenance, not of truth.

Two things about the scoring surfaced today and both are mine.

``answer_is_correct`` asks whether the expected span is inside the
generated one, so it is **generous about additions and strict about
omissions**: "1.75" scores wrong against "1.75 days per month" while
"1.75 days per month, and Berlin is in France" scores right. Half the
wrong answers here are the right fact stated briefly.

And three questions in the paraphrased set are phrased as yes/no while
graded against an extractive span — written that way on Day 6 and
impossible to notice until something started producing sentences. The
retrieval numbers from Days 6-13 are unaffected, since those asked only
whether the gold span was retrieved. The generation numbers are
pessimistic by about 9%. Both are left in place and documented, because a
repo that repairs its evaluation set after seeing the results is not
measuring anything.
"""
...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print("1. an answer that points at its evidence")
    print("=" * 72)
    for question in (QUESTIONS[12], QUESTIONS[4], PARAPHRASED_QUESTIONS[8]):
        result = answer_with_citation(generator, retriever, question.query,
                                      documents, k=3)
        print(f"  Q {question.query!r}")
        print(f"  A {result.answer!r}")
        if result.citation is not None:
            print(f"    {result.citation.title} ({result.citation.uri}) "
                  f"line {result.citation.line}, chars "
                  f"{result.citation.start}..{result.citation.end}")
            print(f"    {result.citation.quote!r}")
        else:
            print("    no supporting chunk found")
        print()
    print("  those offsets are Day 2's, and they still point at the original")
    print("  document rather than at a normalized copy of it.")

    print()
    print("=" * 72)
    print("2. correctness and groundedness are different questions")
    print("=" * 72)
    print(f"  {'k':>3}{'grounded+right':>16}{'grounded+wrong':>16}"
          f"{'ungrounded+right':>18}{'ungrounded+wrong':>18}{'abstained':>11}")
    for k in (2, 3, 8):
        report = grounding_report(generator, retriever, everything, documents, k=k)
        print(f"  {k:>3}{report['grounded_correct']:>16.0%}"
              f"{report['grounded_wrong']:>16.0%}"
              f"{report['ungrounded_correct']:>18.0%}"
              f"{report['ungrounded_wrong']:>18.0%}"
              f"{report['abstained']:>11.0%}")
    report = grounding_report(generator, retriever, everything, documents, k=3)
    grounded = round((report["grounded_correct"] + report["grounded_wrong"])
                     * report["total"])
    print()
    print(f"  every citation verifies: {report['citations_verified']}/{grounded} of the")
    print("  grounded answers point at a span that really does contain what")
    print("  the system says it contains, checked with Day 2's")
    print("  verify_citation against the un-normalized source.")
    print()
    print("  'ungrounded and correct' is 0%. This model does not paraphrase -")
    print("  it lifts spans - so every right answer is traceable. A larger")
    print("  model would fill that cell, and every entry in it would be an")
    print("  answer a user cannot check.")
    print()
    print("  'grounded and wrong' is the cell worth staring at. Those answers")
    print("  quote a real chunk, the quote is accurate, and the answer is")
    print("  still wrong - so the citation makes them MORE convincing rather")
    print("  than less. Citation is a property of provenance, not of truth.")

    print()
    print("=" * 72)
    print("3. what the scorer is doing to those numbers")
    print("=" * 72)
    terse = wrong = 0
    print("  every answer scored wrong at k=3:")
    for question in everything:
        result = answer_with_citation(generator, retriever, question.query,
                                      documents, k=3)
        if result.abstained or answer_is_correct(result.answer, question.answer):
            continue
        if terse_but_right(result.answer, question.answer):
            terse += 1
            label = "TERSE"
        else:
            wrong += 1
            label = "WRONG"
        print(f"    {label}  expected {question.answer!r}")
        print(f"           got      {result.answer!r}")
    print()
    print(f"  {terse} of {terse + wrong} are the right fact stated more briefly than the")
    print("  gold answer. answer_is_correct asks whether the expected span is")
    print("  INSIDE the generated one, which makes it generous about")
    print("  additions and strict about omissions:")
    print()
    print("    '1.75'                                    -> wrong")
    print("    '1.75 days per month, and Berlin is in France' -> right")
    print()
    print("  Day 14 noted the first half of that. This is the second, and")
    print("  together they mean the headline accuracy is understated for")
    print("  terse answers and overstated for padded ones.")

    print()
    print("=" * 72)
    print("4. and a bug in my own evaluation set")
    print("=" * 72)
    print("  three of the remaining WRONG answers are 'yes'. They are")
    print("  answers to questions I wrote in yes/no form on Day 6 and graded")
    print("  against an extractive span:")
    print()
    by_query = {question.query: question for question in everything}
    for query in YES_NO_MISMATCH:
        question = by_query[query]
        print(f"    {query!r}")
        print(f"      graded against: {question.answer!r}")
    print()
    print("  'yes' is a correct answer to 'do I lose untaken time off at year")
    print("  end'. The grader marks it wrong because the gold answer was")
    print("  written for the original phrasing and never updated when the")
    print("  question was rephrased.")
    print()
    print("  this was introduced on Day 6 and could not surface until today,")
    print("  because nothing before Level 4 produced a sentence - Days 6 to")
    print("  13 asked only whether the gold SPAN was in a retrieved chunk,")
    print("  which it still is. So the retrieval numbers stand and the")
    print("  generation numbers on this day and Day 14 are pessimistic by")
    print(f"  about {3 / len(everything):.0%}.")
    print()
    print("  left in place rather than fixed, because a repo that quietly")
    print("  repairs its evaluation set after seeing the results is not")
    print("  measuring anything.")

    print()
    print("=" * 72)
    print("what grounding is worth")
    print("=" * 72)
    print("  it converts an unverifiable claim into a checkable one. That is")
    print("  all, and it is a great deal: a user can follow the citation to")
    print("  a line number in a named document and see for themselves.")
    print()
    print("  what it is NOT is evidence that the answer is right. 18% of")
    print("  answers here are grounded and wrong, with accurate citations")
    print("  attached. Day 16 measures the thing grounding cannot: whether")
    print("  the answer follows from what it cites.")
