...
The first generator in this repo. Everything before today produced a
ranked list; this produces a sentence, and a sentence has to be judged
rather than looked up.

The bottleneck moves here. Day 10 established that dense retrieval puts
the answer in the top ten for **every** question in this set, so any
failure below 100% is now the generator not using what it was handed.

The position experiment holds ten chunks fixed and moves only the one
containing the answer. First position is worth about nine points over
anywhere else — and that is only half the published effect. The famous
result is a U: strong at the start, weak in the middle, recovering at the
end. **There is no recovery here.** The last position scores the same as
the middle, so this model shows a first-position advantage rather than a
U-shape.

That is a different setting rather than a contradiction: the U comes from
decoder-only models with contexts of thousands of tokens, and this is a
248M encoder-decoder attending bidirectionally over 512. Reporting the
U-shape because it is the famous result would be quoting somebody else's
measurement.
"""
...
if __name__ == "__main__":
    ...
    print("=" * 72)
    print("1. the first generator in this repo")
    print("=" * 72)
    print(f"  {DEFAULT_GENERATOR}, {sum(p.numel() for p in generator.model.parameters()) / 1e6:.0f}M"
          f" parameters, {generator.max_input_tokens}-token input limit")
    print()
    context = pack([chunk for chunk, _ in
                    retriever.search("what are the core hours", k=3, method="dense")])
    for question in ("what are the core hours",
        ...
        print(f"      -> {answer!r}")
    print()
    print("  it abstains on the question the context cannot answer, which is")
    print("  the behaviour Day 16 will measure properly. Everything before")
    print("  today produced a ranked list; this produces a sentence, and a")
    print("  sentence has to be judged rather than looked up.")

    print()
    print("=" * 72)
    print("2. how much context to pack")
    print("=" * 72)
    print(f"  {'k':>3}{'correct':>10}{'abstained':>12}{'mean tokens':>13}"
          f"{'max':>7}{'truncated':>11}")
    for k in (1, 2, 3, 5, 8, 12):
        scores = answer_questions(retriever, generator, everything, k=k)
        truncated = "yes" if scores["max_tokens"] > generator.max_input_tokens else "no"
        print(f"  {k:>3}{scores['correct']:>10.0%}{scores['abstained']:>12.0%}"
              f"{scores['mean_tokens']:>13.0f}{scores['max_tokens']:>7}"
              f"{truncated:>11}")
    print()
    print("  Day 10 established that dense retrieval puts the answer in the")
    print("  top ten for every question in this set. So every row here that")
    print("  is below 100% is the generator failing to use something it was")
    print("  given - retrieval is no longer the bottleneck.")
    print()
    print("  and the column does not rise with k. It peaks at two or three")
    print("  chunks and falls back by five, while retrieval recall is going")
    print("  UP the whole way. More context is not more information to this")
    print("  model, it is more to be distracted by.")
    print()
    print("  nothing truncates - the largest prompt here is well inside 512")
    print("  tokens - so this is not the context window being exceeded. It")
    print("  is the model doing worse with material it was successfully")
    print("  given, which is a harder problem than a budget.")

    print()
    print("  and the comparison that puts all of it in perspective:")
    whole = pack(chunks)
    correct = abstain = 0
    for question in everything:
        answer = generator.answer(question.query, whole)
        correct += answer_is_correct(answer, question.answer)
        abstain += abstained(answer)
    total = len(everything)
    print(f"    the ENTIRE handbook is {generator.prompt_tokens('q', whole)} "
          f"tokens, inside the {generator.max_input_tokens}-token limit.")
    print(f"    passing all of it, with no retrieval whatsoever: "
          f"{correct / total:.0%} correct, {abstain / total:.0%} abstained")
    print()
    print("  so retrieval is not NECESSARY on this corpus - everything fits.")
    print("  It is still worth about nine points, and every one of them comes")
    print("  from leaving things out. The answer was always available; what")
    print("  retrieval bought was the absence of the other twenty-four chunks.")
    print()
    print("  worth holding onto for the rest of Level 4: at this size the")
    print("  retriever is a distractor filter rather than a finder, so any")
    print("  claim here about retrieval helping generation is a claim about")
    print("  filtering.")

    print()
    print("=" * 72)
    print("3. the lost-in-the-middle effect, measured")
    print("=" * 72)
    print("  the same ten chunks every time. The only thing that changes is")
    print("  where the chunk holding the answer sits:")
    print()
    print(f"  {'position':>9}{'correct':>10}{'abstained':>12}")
    for position in (0, 2, 4, 6, 9):
        correct = abstain = 0
        for question in everything:
            gold = next((chunk for chunk in chunks if question.answer in chunk.text), None)
            if gold is None:
                continue
            pool = [chunk for chunk, _ in
                    retriever.search(question.query, k=10, method="dense")][:10]
            if gold not in pool:
                pool = pool[:9] + [gold]
            answer = generator.answer(question.query,
                                      pack_with_answer_at(pool, gold, position))
            correct += answer_is_correct(answer, question.answer)
            abstain += abstained(answer)
        total = len(everything)
        print(f"  {position:>9}{correct / total:>10.0%}{abstain / total:>12.0%}")
    print()
    print("  first position is worth about nine points over anywhere else,")
    print("  which is the effect the name refers to - and only half of it.")
    print()
    print("  the published shape is a U: high at the start, low in the")
    print("  middle, recovering at the end. There is no recovery here. The")
    print("  last position scores the same as the middle, so what this model")
    print("  shows is a first-position advantage rather than a U.")
    print()
    print("  that is not a contradiction of the literature, it is a different")
    print("  setting: those results are for decoder-only models with contexts")
    print("  of thousands of tokens, and this is a 248M encoder-decoder whose")
    print("  encoder attends over 512 tokens bidirectionally. Reporting the")
    print("  U-shape here because it is the famous result would be quoting")
    print("  somebody else's measurement.")
    print()
    print(f"  and 34 questions means each percentage point is a third of a")
    print("  question. The nine-point gap is three questions. Day 11's")
    print("  interval covers it, so the honest claim is 'first position looks")
    print("  better, on a sample too small to be sure'.")

    print()
    print("=" * 72)
    print("4. does the order help?")
    print("=" * 72)
    print(f"  at k=8:")
    print(f"  {'order':<12}{'correct':>10}{'abstained':>12}")
    for order in ("relevance", "reversed", "edges"):
        scores = answer_questions(retriever, generator, everything, k=8, order=order)
        print(f"  {order:<12}{scores['correct']:>10.0%}{scores['abstained']:>12.0%}")
    print()
    print("  'edges' is the arrangement the lost-in-the-middle papers suggest:")
    print("  best first, second-best last, the rest buried. On this model it")
    print("  is not better than simply handing over the ranking, and section 3")
    print("  explains why - there is no end-of-context advantage to exploit.")
    print()
    print("  the correct column barely moves. The abstained column does, and")
    print("  that is the result worth keeping: putting the best chunk LAST")
    print("  leaves accuracy alone and multiplies 'not stated' several times")
    print("  over. Order does not change what this model can find so much as")
    print("  how readily it gives up looking.")
    print()
    print("  a system reporting only accuracy would call these three")
    print("  arrangements equivalent. They are not, and the difference is")
    print("  the one a user would notice first.")

    print()
    print("=" * 72)
    print("what this day changes")
    print("=" * 72)
    print("  the bottleneck moved. Levels 2 and 3 spent ten days getting the")
    print("  right chunk into the top ten, and it is there for every question")
    print("  in this set. The generator then answers about two thirds of")
    print("  them.")
    print()
    print("  every remaining failure is a generation failure, and none of it")
    print("  is visible in a retrieval metric. Day 15 makes the answer point")
    print("  at its evidence; Day 16 measures how often it points at nothing.")
