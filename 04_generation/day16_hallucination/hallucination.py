...
"Did the model hallucinate" is a question about meaning, and the usual
answer is to ask a larger model. That replaces one unverified judgement
with another. All three probes here compare the system against **itself**
under a controlled change instead: no judge, and mostly no labels.

**Counterfactual.** Answer, then answer again with the supporting chunk
removed. If the answer does not change, it was not coming from the chunk.
This model recites from its weights **0% of the time** — take the
sentence away and it either stops or says something else, every time.

**Poisoning.** Add one fabricated sentence to the retrieved context. It
repeats every one: core hours of 03:00 to 04:00, an eleven-year laptop
cycle, a three-character password policy — each stated plainly, and each
of which Day 15's machinery would hand over with an accurate citation
pointing at the fabrication.

Those two are the same property from opposite sides. A model that never
answers from memory has nothing to overrule its context with. **Perfect
faithfulness to the context and any defence against a bad context are the
same dial**, which settles what "grounded" is worth: the answer came from
the documents, and that is all it means.

**Abstention**, split by whether the answer was actually there — because
Day 14 reported one rate that pools two mistakes with opposite signs, and
...
if __name__ == "__main__":
    ...
    if not (CROSS_ENCODER_AVAILABLE and GENERATOR_AVAILABLE):
        print("this day needs sentence-transformers and transformers.")
        raise SystemExit(0)
    ...
    print("=" * 72)
    print("the problem with measuring this")
    print("=" * 72)
    print("  'did the model hallucinate' is a question about meaning, and the")
    print("  usual answer is to ask a larger model. That replaces one")
    print("  unverified judgement with another, and this repo has no larger")
    print("  model to ask.")
    print()
    print("  so all three probes below compare the system against ITSELF")
    print("  under a controlled change. None of them needs a judge, and two")
    print("  of them need no labels either.")

    print()
    print("=" * 72)
    print("1. counterfactual: take the evidence away")
    print("=" * 72)
    print("  answer normally, then answer again with the supporting chunk")
    print("  removed. If the answer does not change, it was not coming from")
    print("  the chunk.")
    print()
    report = counterfactual_faithfulness(generator, retriever, everything, k=3)
    for name, meaning in (("abstained_without", "stopped answering - faithful"),
                          ("changed_without", "answered differently - faithful"),
                          ("unchanged_without", "same answer, no evidence - recited"),
                          ("no_support", "nothing to remove")):
        print(f"    {name:<20}{report[name]:>7.0%}   {meaning}")
    print()
    print(f"  recited from weights: {report['unchanged_without']:.0%}.")
    print()
    print("  this model does not answer from memory. Take the sentence away")
    print("  and it either stops or says something else, every time. On a")
    print("  private corpus that is the property you actually want, because")
    print("  an answer produced without the document is an answer the")
    print("  document cannot justify - and Day 15's citation would be")
    print("  decorating it anyway.")

    print()
    print("=" * 72)
    print("2. poisoning: put a false fact in the context")
    print("=" * 72)
    print("  the same questions, with one fabricated sentence added to the")
    print("  retrieved context. Which source wins?")
    print()
    poisoned = poisoning_report(generator, retriever, k=3)
    for row in poisoned["rows"]:
        print(f"    {row['query']!r}")
        print(f"      -> {row['answer']!r}   [{row['verdict']}]")
    print()
    print(f"  followed the context {poisoned['followed']:.0%}, "
          f"kept the true fact {poisoned['overruled']:.0%}, "
          f"abstained {poisoned['other']:.0%}")
    print()
    print("  it repeats every one. Core hours of 03:00 to 04:00, a laptop")
    print("  cycle of eleven years, a three-character password policy - all")
    print("  stated plainly, and on Day 15's machinery all of them would")
    print("  arrive with an accurate citation pointing at the fabricated")
    print("  sentence.")
    print()
    print("  and this is the SAME property as section 1, seen from the other")
    print("  side. A model that never answers from memory has nothing to")
    print("  overrule its context with. Perfect faithfulness to the context")
    print("  and any defence against a bad context are the same dial.")
    print()
    print("  which settles what 'grounded' is worth: it means the answer")
    print("  came from the documents. It does not mean the documents were")
    print("  right, and no amount of grounding machinery can make it mean")
    print("  that.")

    print()
    print("=" * 72)
    print("3. abstention, split by whether the answer was there")
    print("=" * 72)
    print("  Day 14 reported an abstention rate. That number pools two")
    print("  mistakes with opposite signs:")
    print()
    abstention = abstention_report(generator, retriever, everything, k=3)
    print(f"    answerable questions    {abstention['answerable']:>3}"
          f"   wrongly abstained {abstention['wrongly_abstained']:>5.0%}")
    print(f"    unanswerable questions  {abstention['unanswerable']:>3}"
          f"   wrongly answered  {abstention['wrongly_answered']:>5.0%}")
    print()
    print("  the unanswerable set is six questions the handbook does not")
    print("  cover, written in the same register so that retrieval still")
    print("  returns three plausible chunks and declining has to be the")
    print("  model's decision rather than an empty context:")
    for query in UNANSWERABLE[:3]:
        answer = generator.answer(query, pack([chunk for chunk, _ in
                                               retriever.search(query, k=3,
                                                                method="dense")]))
        print(f"    {query!r}")
        print(f"      -> {answer!r}")
    print()
    print("  both error types are present and neither dominates. A single")
    print("  abstention rate would have hidden that, and the two have")
    print("  different fixes: one wants a more willing model, the other a")
    print("  more cautious one, and they pull against each other.")

    print()
    print("=" * 72)
    print("what this day measured, and what it could not")
    print("=" * 72)
    print("  measured, with no judge model and mostly no labels:")
    print("    - whether an answer depends on its evidence (it does, 100%)")
    print("    - whether the model prefers its context to the truth")
    print("      (the context, 100%)")
    print("    - whether it declines at the right times (mostly, in both")
    print("      directions)")
    print()
    print("  not measured: whether an answer is FAITHFUL in the sense people")
    print("  mean - that it follows from the context without adding,")
    print("  distorting or over-claiming. Day 15's containment check catches")
    print("  an answer absent from the context; it cannot catch one that is")
    print("  present and subtly misused, and the 18% grounded-and-wrong cell")
    print("  from that day is exactly that population.")
    print()
    print("  a judge model would give a number for it. It would be an")
    print("  unverified number from an unverified judge, and this repo has")
    print("  spent sixteen days not doing that.")
