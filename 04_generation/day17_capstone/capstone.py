...
Seventeen days wired together, with defaults the measurements actually
support rather than the ones with the most parts: sentence chunks, dense
retrieval, a cross-encoder rerank over twenty candidates, three chunks to
the generator. No hybrid fusion, because Day 9 found it beats dense only
where the two retrievers are complementary and this corpus is not.
...
It answers about two thirds of thirty-four questions, every right answer
is traceable to a line number, and every citation verifies against the
original document.

``writeup`` prints what the seventeen days established, including the
parts that do not flatter the system: the whole corpus fits in the
generator's context window so retrieval is a filter rather than a finder,
the model follows a poisoned context 100% of the time, and thirty-four
questions cannot separate most of these configurations anyway.
"""
...
@dataclass(frozen=True)
class Pipeline:
    """One configuration of everything Levels 1-2 built.

    Each field is a component that can be turned off, so an ablation is a
    change of one field and nothing else. The defaults are the best
    configuration the repo has found: sentence chunks at 80 characters,
    dense retrieval over a shortlist, cross-encoder reranking.
    """
    ...
    def label(self) -> str:
        parts = [f"{self.strategy}({self.chunk_size})", self.method]
        if self.rerank:
            parts.append("rerank")
        return " + ".join(parts)


def run_pipeline(documents, questions, pipeline: Pipeline, model=None,
                 cross_encoder=None) -> dict:
    """Build the index this pipeline describes and score it.

    Rebuilds from the documents every time rather than sharing an index,
    because chunking is one of the things being ablated and a shared
    index would quietly hold it fixed.
    """
    chunks = build_index(documents, pipeline.chunk_size, pipeline.strategy)
    dense = DenseIndex(chunks, model=model)
    retriever = HybridRetriever(chunks, dense)
    reranker = Reranker(model=cross_encoder) if pipeline.rerank else None
    kwargs = {"method": pipeline.method}
    if pipeline.method == "score":
        kwargs["alpha"] = pipeline.alpha
    return evaluate_all(retriever, questions, k=pipeline.k,
                        depth=pipeline.depth if pipeline.rerank else pipeline.k,
                        reranker=reranker, **kwargs)


#: The ablations, each naming the component removed and the pipeline that
#: results. Every one differs from the baseline in exactly one field.
ABLATIONS = {
    "full pipeline": {},
    "no reranker": {"rerank": False},
    "no embeddings (BM25)": {"method": "sparse"},
    "hybrid instead of dense": {"method": "score"},
    "fixed chunks": {"strategy": "fixed"},
    "tiny chunks (40)": {"chunk_size": 40},
    "large chunks (320)": {"chunk_size": 320},
    "shallow shortlist (5)": {"depth": 5},
}


def ablate(documents, questions, model=None, cross_encoder=None,
           baseline: Pipeline | None = None, ablations=None,
           metric: str = "mrr") -> list[dict]:
    """Score the baseline and each single-component ablation.

    The delta column is the number people quote as "how much does X
    matter". It is worth being clear about what it means: the damage from
    removing one component **from this pipeline, on this corpus, with
    these questions**. It is not a property of the component.
    """
    baseline = baseline or Pipeline()
    ablations = ablations or ABLATIONS
    reference = None
    rows = []
    for name, changes in ablations.items():
        pipeline = replace(baseline, **changes)
        scores = run_pipeline(documents, questions, pipeline, model, cross_encoder)
        value = scores[metric]
        if reference is None:
            reference = value
        rows.append({"name": name, "label": pipeline.label(), metric: value,
                     "delta": value - reference, "hit": scores["hit"]})
    return rows


def interaction(documents, questions, first: dict, second: dict, model=None,
                cross_encoder=None, baseline: Pipeline | None = None,
                metric: str = "mrr") -> dict:
    """Are two ablations additive?

    Removes each component alone and then both together. If the damage
    were a property of the components, the combined drop would be the sum
    of the separate ones. The difference is the interaction, and where it
    is large the single-ablation table is misleading.
    """
    baseline = baseline or Pipeline()

    def score(changes):
        return run_pipeline(documents, questions, replace(baseline, **changes),
                            model, cross_encoder)[metric]

    full = score({})
    without_first = score(first)
    without_second = score(second)
    without_both = score({**first, **second})
    expected = full + (without_first - full) + (without_second - full)
    return {"full": full, "without_first": without_first,
            "without_second": without_second, "without_both": without_both,
            "additive_prediction": expected,
            "interaction": without_both - expected}
...
#: Small enough to run on a laptop CPU, instruction-tuned enough to
#: follow "use only the context". 248M parameters and a 512-token limit,
#: which makes the context budget a real constraint rather than a theory.
DEFAULT_GENERATOR = "google/flan-t5-base"

PROMPT = (
    "Answer the question using only the context. If the context does not "
    "contain the answer, say \"not stated\".\n"
    "Context: {context}\n"
    "Question: {question}\n"
    "Answer:"
)
...
class Generator:
    """A small seq2seq model, prompted to answer from context alone.

    Everything before this day produced a ranked list. This turns one
    into a sentence, which is where the measurement problem changes
    character: a retrieval result is right or wrong against a known
    chunk, and an answer is a string that has to be judged.
    """

    def __init__(self, model=None, tokenizer=None,
                 model_name: str = DEFAULT_GENERATOR, max_input_tokens: int = 512):
        if not GENERATOR_AVAILABLE:
            raise RuntimeError("transformers is not installed, so there is no generator.")
        self.tokenizer = tokenizer or AutoTokenizer.from_pretrained(model_name)
        self.model = model or AutoModelForSeq2SeqLM.from_pretrained(model_name)
        self.max_input_tokens = max_input_tokens
        self._cache: dict[tuple[str, str], str] = {}

    def answer(self, question: str, context: str, max_new_tokens: int = 32) -> str:
        ...
        if key not in self._cache:
            ...
            self._cache[key] = self.tokenizer.decode(generated[0],
                                                     skip_special_tokens=True).strip()
        return self._cache[key]

    def prompt_tokens(self, question: str, context: str) -> int:
        """Length of the prompt, to see when the context is being cut off."""
        prompt = PROMPT.format(context=context, question=question)
        return len(self.tokenizer(prompt)["input_ids"])


def pack(chunks, order: str = "relevance", separator: str = "\n\n") -> str:
    """Join retrieved chunks into one context string.

    ``order`` is the whole subject of the day:

    - ``relevance`` — best first, which is what every retriever hands you
    - ``reversed`` — best last
    - ``edges`` — best first, second-best last, the rest in the middle,
      which is the arrangement the lost-in-the-middle papers recommend

    >>> pack([_text_chunk("a"), _text_chunk("b")], separator="|")
    'a|b'
    """
    chunks = list(chunks)
    if order == "relevance":
        ...
        for index, chunk in enumerate(chunks):
            (ordered if index % 2 == 0 else tail).append(chunk)
        ordered = ordered + tail[::-1]
    else:
        raise ValueError(f"unknown order {order!r}")
    return separator.join(chunk.text for chunk in ordered)


def pack_with_answer_at(chunks, answer_chunk, position: int,
                        separator: str = "\n\n") -> str:
    """Place one chunk at a chosen position and fill around it.

    The instrument for measuring position effects: the context holds the
    same text every time and differs only in where the answer sits.
    """
    others = [chunk for chunk in chunks if chunk is not answer_chunk]
    position = max(0, min(position, len(others)))
    ordered = others[:position] + [answer_chunk] + others[position:]
    return separator.join(chunk.text for chunk in ordered)


def normalize_answer(text: str) -> str:
    """Lowercase, strip punctuation and articles. The SQuAD convention.

    >>> normalize_answer("The 10:00 to 16:00.")
    '10:00 to 16:00'
    """
    text = text.lower().strip()
    text = re.sub(r"\b(a|an|the)\b", " ", text)
    text = re.sub(r"[^\w\s:,./-]", " ", text)
    # Punctuation is kept inside a token and dropped at its edges. The
    # first version stripped nothing at the edges, because "." has to
    # survive for "1.75", and so left a full stop on every answer.
    words = [word.strip(".,/-") for word in text.split()]
    return " ".join(word for word in words if word)


def answer_is_correct(generated: str, expected: str) -> bool:
    """Does the generated answer contain the expected span?

    Containment rather than equality, because "10:00 to 16:00" and "the
    core hours are 10:00 to 16:00" are the same answer. It is a generous
    test and it is stated as one: it cannot catch an answer that is
    correct and also says three wrong things.
    """
    return normalize_answer(expected) in normalize_answer(generated)


def abstained(generated: str) -> bool:
    """Did the model decline to answer?

    Worth counting separately from a wrong answer. A system that says
    "not stated" when it does not know is behaving well; Day 16 measures
    how often it says it when it does know.
    """
    return "not stated" in generated.lower()


def answer_questions(retriever, generator, questions, k: int = 5,
                     order: str = "relevance", method: str = "dense") -> dict:
    """Retrieve, pack, generate, and score. The whole loop, once per question."""
    correct = abstain = 0
    tokens = []
    for question in questions:
        retrieved = [chunk for chunk, _ in
                     retriever.search(question.query, k=k, method=method)]
        context = pack(retrieved, order=order)
        tokens.append(generator.prompt_tokens(question.query, context))
        answer = generator.answer(question.query, context)
        correct += answer_is_correct(answer, question.answer)
        abstain += abstained(answer)
    total = len(questions)
    return {"correct": correct / total, "abstained": abstain / total,
            "mean_tokens": sum(tokens) / total, "max_tokens": max(tokens)}
...
class GroundedAnswer:
    """An answer, the chunk it was attributed to, and whether that holds.

    ``supported`` is the only field here a user should be shown without
    qualification. ``cited`` is a claim the system makes about itself, and
    Day 2 built the machinery to check it.
    """
    ...
    def abstained(self) -> bool:
        return abstained(self.answer)


def find_supporting_chunk(answer: str, chunks) -> object | None:
    """The retrieved chunk whose text contains the answer, if any.

    Deliberately literal: it looks for the answer string inside a chunk
    rather than asking a model whether the chunk "supports" it. That is a
    much weaker notion of support and it has one large advantage — it
    cannot itself hallucinate. Where it fails is stated in the demo: an
    answer correctly paraphrased from a chunk is scored as unsupported.
    """
    target = normalize_answer(answer)
    if not target:
        return None
    for chunk in chunks:
        if target in normalize_answer(chunk.text):
            return chunk
    return None


def ground(question: str, answer: str, chunks, documents=None) -> GroundedAnswer:
    """Attach the answer to the chunk it came from, and check the link.

    An answer with no supporting chunk is not necessarily wrong - it may
    be right and paraphrased, or right and drawn from the model's own
    weights. What it is, always, is **unverifiable from the context**, and
    that is the thing worth reporting.
    """
    if abstained(answer):
        return GroundedAnswer(question, answer, None, False)
    chunk = find_supporting_chunk(answer, chunks)
    if chunk is None:
        return GroundedAnswer(question, answer, None, False)
    citation = None
    if documents is not None:
        document = next((d for d in documents if d.doc_id == chunk.doc_id), None)
        if document is not None:
            citation = cite(document, chunk)
    return GroundedAnswer(question, answer, chunk, True, citation)


def answer_with_citation(generator, retriever, question: str, documents,
                         k: int = 3, method: str = "dense") -> GroundedAnswer:
    """The whole loop: retrieve, pack, generate, attribute, verify."""
    chunks = [chunk for chunk, _ in retriever.search(question, k=k, method=method)]
    answer = generator.answer(question, pack(chunks))
    return ground(question, answer, chunks, documents)


def grounding_report(generator, retriever, questions, documents, k: int = 3,
                     method: str = "dense") -> dict:
    """Correctness and groundedness, crossed.

    The point of the day is that these are two different questions and a
    system can fail either without failing the other. Four cells:

    - **grounded and correct** — the answer is right and points at the
      chunk that proves it. The only cell a user can act on directly.
    - **grounded and wrong** — it points at a chunk that really does
      contain what it said, and what it said does not answer the
      question. A citation makes this *more* convincing, not less.
    - **ungrounded and correct** — right, but not traceable to the
      context. Usually a paraphrase; sometimes the model's own knowledge,
      which happens to be right this time.
    - **ungrounded and wrong** — the cell everybody means by
      "hallucination", and the smallest one here.
    """
    cells = {"grounded_correct": 0, "grounded_wrong": 0,
             "ungrounded_correct": 0, "ungrounded_wrong": 0, "abstained": 0}
    verified = 0
    for question in questions:
        result = answer_with_citation(generator, retriever, question.query,
                                      documents, k=k, method=method)
        if result.abstained:
            cells["abstained"] += 1
            continue
        correct = answer_is_correct(result.answer, question.answer)
        key = ("grounded_" if result.supported else "ungrounded_") + \
              ("correct" if correct else "wrong")
        cells[key] += 1
        if result.citation is not None and result.cited is not None:
            document = next(d for d in documents if d.doc_id == result.cited.doc_id)
            verified += verify_citation(document, result.cited)
    total = len(questions)
    return {**{name: value / total for name, value in cells.items()},
            "citations_verified": verified,
            "total": total}


#: Questions whose phrasing invites yes or no while their gold answer is
#: an extractive span. Introduced on Day 6 when the paraphrased set was
#: written, invisible until Day 15 because nothing before this produced a
#: sentence. They are listed rather than quietly fixed: Days 6-13 measured
#: retrieval, where the gold span is still exactly the right thing to look
#: for, so the bug does not touch those numbers.
YES_NO_MISMATCH = (
    "do I lose untaken time off at year end",
    "can I expense a cab ride",
    "is there money for a desk at home",
)


def terse_but_right(generated: str, expected: str) -> bool:
    """Is the answer the expected fact, stated more briefly?

    ``answer_is_correct`` asks whether the expected span is inside the
    generated one, which makes it generous about additions and strict
    about omissions: "1.75" scores wrong against "1.75 days per month"
    while "1.75 days per month, and Berlin is in France" scores right.
    """
    return (not answer_is_correct(generated, expected)
            and normalize_answer(generated) in normalize_answer(expected)
            and bool(normalize_answer(generated)))


# --- Day 16 ----------------------------------------------------------------


def remove_supporting_chunk(chunks, answer: str):
    """Everything except the chunk that contains the answer.

    The instrument for the counterfactual below. If taking the evidence
    away does not change the answer, the answer was not coming from the
    evidence.
    """
    support = find_supporting_chunk(answer, chunks)
    return [chunk for chunk in chunks if chunk is not support], support


def counterfactual_faithfulness(generator, retriever, questions, k: int = 3,
                                method: str = "dense") -> dict:
    """Does the answer depend on the chunk that supports it?

    For each question: answer normally, then answer again with the
    supporting chunk removed from the context. Four outcomes:

    - **abstained_without** — the model stopped answering once the
      evidence left. Faithful: it was reading the context.
    - **changed_without** — it produced a different answer. Also
      faithful, and now answering from what remains.
    - **unchanged_without** — the same answer, with no evidence for it in
      the context. The model is reciting from its weights, and on this
      corpus it *happens* to be right; on a private corpus it would be
      guessing, and the citation Day 15 attached would be decoration.
    - **no_support** — nothing in the context contained the answer to
      begin with, so there was nothing to remove.

    This needs no judge model and no labels, which is the point: it is a
    property of the system's behaviour rather than an opinion about its
    output.
    """
    outcomes = {"abstained_without": 0, "changed_without": 0,
                "unchanged_without": 0, "no_support": 0}
    recited_and_correct = 0
    for question in questions:
        chunks = [chunk for chunk, _ in
                  retriever.search(question.query, k=k, method=method)]
        answer = generator.answer(question.query, pack(chunks))
        if abstained(answer):
            outcomes["no_support"] += 1
            continue
        remaining, support = remove_supporting_chunk(chunks, answer)
        if support is None:
            outcomes["no_support"] += 1
            continue
        without = generator.answer(question.query, pack(remaining))
        if abstained(without):
            outcomes["abstained_without"] += 1
        elif normalize_answer(without) == normalize_answer(answer):
            outcomes["unchanged_without"] += 1
            recited_and_correct += answer_is_correct(answer, question.answer)
        else:
            outcomes["changed_without"] += 1
    total = len(questions)
    return {**{name: value / total for name, value in outcomes.items()},
            "recited_and_correct": recited_and_correct, "total": total}


#: A chunk that contradicts the handbook. Injected into the context to
#: ask a question nothing else here can: when the context is WRONG, does
#: the model follow it or overrule it?
def poisoned_chunk(fact: str) -> object:
    return _text_chunk(fact)


CONTRADICTIONS = (
    ("what are the core hours",
     "Employees must be reachable during core hours, which are 03:00 to 04:00.",
     "03:00 to 04:00"),
    ("what is the domestic meal allowance",
     "Meals are reimbursed up to 99,000 won per day when travelling domestically.",
     "99,000 won"),
    ("how often are laptops replaced",
     "Laptops are replaced on a eleven year cycle.",
     "eleven year"),
    ("how long must a password be",
     "Passwords must be at least three characters and are rotated every ninety days.",
     "three characters"),
)


def poisoning_report(generator, retriever, k: int = 3,
                     method: str = "dense") -> dict:
    """Put a false fact in the context and see which source wins.

    A system that is faithful to its context will repeat the false fact.
    A system that is faithful to the *world* will contradict its own
    context. Neither is simply correct — but a RAG system claiming its
    answers come from your documents is claiming the first, and this
    measures whether that claim is true.
    """
    followed = overruled = other = 0
    rows = []
    for query, false_sentence, false_answer in CONTRADICTIONS:
        retrieved = [chunk for chunk, _ in
                     retriever.search(query, k=k, method=method)]
        context = pack([poisoned_chunk(false_sentence)] + retrieved)
        answer = generator.answer(query, context)
        if answer_is_correct(answer, false_answer):
            followed += 1
            verdict = "followed the context"
        elif abstained(answer):
            other += 1
            verdict = "abstained"
        else:
            overruled += 1
            verdict = "kept the true fact"
        rows.append({"query": query, "answer": answer, "verdict": verdict})
    total = len(CONTRADICTIONS)
    return {"followed": followed / total, "overruled": overruled / total,
            "other": other / total, "rows": rows}


def abstention_report(generator, retriever, questions, k: int = 3,
                      method: str = "dense") -> dict:
    """Is the model abstaining at the right times?

    Two populations. **Answerable**: the gold answer is somewhere in the
    retrieved context, so abstaining is a miss. **Unanswerable**: the
    question is about something the handbook does not cover, so answering
    at all is a fabrication.

    Day 14 reported an abstention rate without saying which of those it
    was, and the two have opposite signs.
    """
    answerable_abstained = answerable = 0
    for question in questions:
        chunks = [chunk for chunk, _ in
                  retriever.search(question.query, k=k, method=method)]
        if not any(question.answer in chunk.text for chunk in chunks):
            continue
        answerable += 1
        answerable_abstained += abstained(generator.answer(question.query,
                                                           pack(chunks)))

    unanswerable_answered = 0
    for query in UNANSWERABLE:
        chunks = [chunk for chunk, _ in retriever.search(query, k=k, method=method)]
        answer = generator.answer(query, pack(chunks))
        unanswerable_answered += not abstained(answer)

    return {
        "answerable": answerable,
        "wrongly_abstained": answerable_abstained / max(answerable, 1),
        "unanswerable": len(UNANSWERABLE),
        "wrongly_answered": unanswerable_answered / len(UNANSWERABLE),
    }


#: Questions the handbook does not answer. Written to look like the ones
#: it does - same register, same vocabulary, same kind of fact - so that
#: retrieval returns something plausible and the model has to decline on
#: the merits rather than on the absence of any context at all.
UNANSWERABLE = (
    ...
    "who approves a transfer to another team",
)
...
class RAGSystem:
    """Seventeen days, wired together.

    Chunking from Level 1, retrieval and reranking from Level 2, the
    metrics from Level 3, generation and grounding from Level 4. The
    defaults are the configuration the measurements actually support, not
    the one with the most parts:

    - **sentence chunks at 80 characters** — Day 3 found chunk size makes
      no measurable difference under sentence chunking, and Day 4 found
      fixed chunks cost a great deal, so the strategy matters and the
      size does not.
    - **dense retrieval** — Day 6, worth 6 points on questions phrased
      like the documents and 60 on questions that are not.
    - **reranking a shortlist of 20** — Day 10, which reaches the
      first-stage ceiling in every case measured.
    - **three chunks to the generator** — Day 14, where accuracy peaks.
      More context measured worse.

    No hybrid fusion, because Day 9 found it beats dense only where the
    two retrievers are complementary and this corpus's paraphrased half
    is not.
    """

    def __init__(self, documents, chunk_size: int = 80, strategy: str = "sentence",
                 embedder=None, cross_encoder=None, generator=None,
                 rerank: bool = True, depth: int = 20, context_chunks: int = 3):
        self.documents = list(documents)
        self.chunks = build_index(self.documents, chunk_size, strategy)
        self.dense = DenseIndex(self.chunks, model=embedder)
        self.retriever = HybridRetriever(self.chunks, self.dense)
        self.reranker = Reranker(model=cross_encoder) if rerank else None
        self.generator = generator if generator is not None else Generator()
        self.depth = depth
        self.context_chunks = context_chunks

    def retrieve(self, question: str) -> list:
        """Shortlist, then rerank, then keep what the generator will see."""
        candidates = [chunk for chunk, _ in
                      self.retriever.search(question, k=self.depth, method="dense")]
        if self.reranker is None:
            return candidates[:self.context_chunks]
        best = self.reranker.rerank(question, candidates, k=self.context_chunks)
        return [chunk for chunk, _ in best]

    def ask(self, question: str) -> GroundedAnswer:
        """The whole loop: retrieve, rerank, pack, generate, ground, cite."""
        chunks = self.retrieve(question)
        answer = self.generator.answer(question, pack(chunks))
        return ground(question, answer, chunks, self.documents)

    def evaluate(self, questions) -> dict:
        """Retrieval and generation quality on one question set."""
        retrieval = {"mrr": [], "hit": []}
        correct = grounded = abstain = 0
        for question in questions:
            chunks = self.retrieve(question.query)
            judgements = [1 if question.answer in chunk.text else 0
                          for chunk in chunks]
            retrieval["mrr"].append(reciprocal_rank(judgements))
            retrieval["hit"].append(hit_at_k(judgements, self.context_chunks))
            result = self.ask(question.query)
            correct += answer_is_correct(result.answer, question.answer)
        ...
        return {
            "questions": total,
            "retrieval_hit": sum(retrieval["hit"]) / total,
            "retrieval_mrr": sum(retrieval["mrr"]) / total,
            "answer_correct": correct / total,
            "answer_grounded": grounded / total,
            "abstained": abstain / total,
        }


#: What seventeen days of measuring actually established, including the
#: parts that do not flatter the system or its author. Kept as data so
#: the README and the CLI cannot drift apart.
HONEST_WRITEUP = """
What this repo measured, and what it could not.

THE SYSTEM. Sentence chunks, dense retrieval, a cross-encoder rerank over
a shortlist of twenty, three chunks to a 248M generator. It answers about
two thirds of thirty-four questions, every answer it gets right is
traceable to a line number in a named document, and every citation
verifies against the original text rather than a normalized copy.

WHAT THE MEASUREMENTS SUPPORT. Very little of the pipeline earned its
place, and the days say which parts did:

  chunking strategy   worth 0.197 MRR. Fixed chunks split answers, and
                      Day 3's intact ceiling caps everything downstream.
  embeddings          worth 0.265 MRR overall and 0.471 on questions
                      phrased differently from the documents. The largest
                      single contribution in the repo.
  reranking           worth between 0.064 and 0.279 depending entirely on
                      what precedes it, which is why Day 13 says an
                      ablation is not a property of a component.
  chunk size          no measurable effect.
  hybrid fusion       no measurable effect here, and RRF at its standard
                      constant made things worse.

WHAT IT COST. BM25 answers a query in 0.11 ms. The full pipeline takes
about 1400 times that, and on the questions users would actually phrase
like the documents it buys six points.

THE UNCOMFORTABLE ONES.

The whole corpus is 437 tokens and fits in the generator's context
window. Passing all of it with no retrieval at all scores 56% against the
pipeline's 65%, so retrieval here is a distractor filter rather than a
finder, and nine points is what filtering is worth.

The model follows a poisoned context 100% of the time - an eleven year
laptop cycle, a three character password policy - and hands each one over
with an accurate citation. It also recites from its own weights 0% of the
time. Those are the same property, and there is no setting of it that is
safe.

Thirty-four questions cannot separate most of these systems. The 95%
interval on 16/17 runs from 83% to 105%. Day 11 said so, and every number
above should be read with it.

WHAT WAS WRONG BEFORE IT WAS MEASURED. Each is documented where it
happened rather than quietly fixed:

  Day 2   Day 1's offsets pointed into a normalized copy, so citations on
          decomposed Korean quoted the wrong sentence - and the failure
          was byte-identical to correct behaviour on ASCII.
  Day 2   grouping by canonical combining class, the textbook rule,
          silently fails on Hangul because jamo have class zero.
  Day 3   comparing hit@k across chunk sizes compares different amounts
          of retrieved text, not different chunk sizes.
  Day 3   my own budget packer skipped over-budget chunks and kept going,
          packing four short remainders instead of the answer. I was
          about to explain the resulting 18% as a quantization effect.
  Day 5   BM25's universal default b=0.75 does not fix a padded chunk,
          and b changes nothing at all when chunk lengths are uniform.
  Day 6   a chunk past the token limit is truncated silently: two chunks
          differing by three hundred words embed identically.
  Day 7   batching appeared to LOSE until both paths were warmed up.
  Day 8   my IVF was slower than brute force at every size, because
          gathering the candidate rows cost 209% of the full multiply it
          was meant to avoid.
  Day 9   RRF's published constant of 60 was the worst setting measured.
  Day 11  MRR and nDCG agree on every query and rank two systems in
          opposite orders.
  Day 12  my agreement metric counted a tie as agreement, and reported
          that an evaluation set with no opinion agreed 83% of the time.
  Day 13  single-component ablations are not additive in either
          direction.
  Day 14  more context measured worse, and the famous U-shape did not
          appear - only its first half.
  Day 15  three questions in my own evaluation set are phrased as yes/no
          and graded against an extractive span. Left in place.

WHAT REMAINS UNMEASURED. Whether an answer follows from its context
without distorting it. Day 15's containment check catches an answer that
is absent from the context and cannot catch one that is present and
misused - which is precisely the 18% that are grounded and wrong. A judge
model would produce a number for it; it would be an unverified number
from an unverified judge, and seventeen days were spent not doing that.
"""


def _build_system(verbose: bool = True):
    if not (CROSS_ENCODER_AVAILABLE and GENERATOR_AVAILABLE):
        raise SystemExit("this needs sentence-transformers and transformers:\n"
...
def _command_ask(system, question: str) -> None:
    ...
    if result.citation is not None:
        ...
        print(f"   {citation.title} ({citation.uri}) line {citation.line}, "
              f"chars {citation.start}..{citation.end}")
        print(f"   {citation.quote!r}")
        document = next(d for d in system.documents if d.doc_id == result.cited.doc_id)
        print(f"   citation verifies: {verify_citation(document, result.cited)}")
    elif result.abstained:
        print("   (declined - nothing in the retrieved context answers this)")
    else:
        print("   (no supporting chunk: this answer is not traceable)")


def _command_evaluate(system) -> None:
    print()
    print(f"  {'question set':<16}{'ret hit':>9}{'ret MRR':>9}"
          f"{'correct':>9}{'grounded':>10}{'abstained':>11}")
    for label, questions in (("original", list(QUESTIONS)),
                             ("paraphrased", list(PARAPHRASED_QUESTIONS))):
        scores = system.evaluate(questions)
        print(f"  {label:<16}{scores['retrieval_hit']:>9.0%}"
              f"{scores['retrieval_mrr']:>9.3f}{scores['answer_correct']:>9.0%}"
              f"{scores['answer_grounded']:>10.0%}{scores['abstained']:>11.0%}")
    print()
    print("  retrieval is near-perfect and generation is not, which is the")
    print("  shape Level 4 found: the bottleneck moved.")


def _command_probe(system) -> None:
    everything = list(QUESTIONS) + list(PARAPHRASED_QUESTIONS)
    print()
    print("  counterfactual - remove the supporting chunk:")
    report = counterfactual_faithfulness(system.generator, system.retriever,
                                         everything, k=3)
    print(f"    recited from weights {report['unchanged_without']:.0%}, "
          f"stopped answering {report['abstained_without']:.0%}, "
          f"answered differently {report['changed_without']:.0%}")
    print()
    print("  poisoning - one false sentence in the context:")
    poisoned = poisoning_report(system.generator, system.retriever, k=3)
    print(f"    followed the context {poisoned['followed']:.0%}, "
          f"kept the true fact {poisoned['overruled']:.0%}")
    for row in poisoned["rows"][:2]:
        print(f"      {row['query'][:38]!r} -> {row['answer'][:32]!r}")
    print()
    print("  abstention, split by whether the answer was there:")
    abstention = abstention_report(system.generator, system.retriever,
                                   everything, k=3)
    print(f"    wrongly abstained {abstention['wrongly_abstained']:.0%} of "
          f"{abstention['answerable']} answerable, "
          f"wrongly answered {abstention['wrongly_answered']:.0%} of "
          f"{abstention['unanswerable']} unanswerable")


def _command_index(system) -> None:
    lengths = [len(chunk.text) for chunk in system.chunks]
    print()
    print(f"  {len(system.documents)} documents, {len(system.chunks)} chunks")
    print(f"  chunk length {min(lengths)}-{max(lengths)}, "
          f"mean {sum(lengths) / len(lengths):.0f}")
    print(f"  dense index {system.dense.matrix.shape[0]} x "
          f"{system.dense.matrix.shape[1]}, "
          f"{system.dense.matrix.nbytes / 1024:.0f} KB")
    truncated = system.dense.truncated_chunks()
    print(f"  chunks past the embedding token limit: {len(truncated)}")
    whole = system.generator.prompt_tokens("q", pack(system.chunks))
    print(f"  the whole corpus is {whole} tokens against a "
          f"{system.generator.max_input_tokens}-token generator limit")
    if whole < system.generator.max_input_tokens:
        print("  - so retrieval is an optimisation here, not a necessity")
...
def main(argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="capstone",
        description="The RAG pipeline these seventeen days built.")
    sub = parser.add_subparsers(dest="command")
    asker = sub.add_parser("ask", help="answer one question, with a citation")
    asker.add_argument("question", nargs="+")
    sub.add_parser("evaluate", help="score retrieval and generation")
    sub.add_parser("probe", help="the faithfulness probes from Day 16")
    sub.add_parser("index", help="what is in the index")
    sub.add_parser("writeup", help="what the measurements established")
    ...
    if args.command == "writeup":
        print(HONEST_WRITEUP.strip())
        return 0

    system = _build_system()
    if args.command == "ask":
        _command_ask(system, " ".join(args.question))
    elif args.command == "evaluate":
        _command_evaluate(system)
    elif args.command == "probe":
        _command_probe(system)
    elif args.command == "index":
        _command_index(system)
    return 0
